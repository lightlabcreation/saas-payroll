...
const getDashboard = async (req, res, next) => {
  try {
    ...
    // Get specific SaaS metrics for Trials vs Paid
    const [[{ count: activeTrials }]] = await db.query(`
      SELECT COUNT(DISTINCT employer_id) as count 
      FROM subscriptions 
      WHERE plan_id = 1 AND status = 'active' AND end_date >= NOW()
    `);

    const [[{ count: expiredTrials }]] = await db.query(`
      SELECT COUNT(DISTINCT employer_id) as count 
      FROM subscriptions 
      WHERE plan_id = 1 AND (status = 'expired' OR end_date < NOW())
    `);

    const [[{ count: paidCustomers }]] = await db.query(`
      SELECT COUNT(DISTINCT employer_id) as count 
      FROM subscriptions 
      WHERE plan_id > 1 AND status = 'active' AND end_date >= NOW()
    `);
